...
namespace deobf::ast::ir {
	...
	struct node : public std::enable_shared_from_this<node> {
		...
		std::vector<std::reference_wrapper<node_type>> get_children() const {
			...
			for (const auto& current_node : children) {
				...
				const auto next_children = current_node.get()->get_children();
				std::move(results.begin(), results.end(), std::back_inserter(results));
			}
