...
namespace deobf::ironbrew_devirtualizer::symbolic_execution {
	...
	*/
	...
	struct loop_unrolled_bst final : ir::abstract_visitor_pattern {
		...
		explicit loop_unrolled_bst(ir::statement::if_statement* const root, std::deque<std::reference_wrapper<vm_arch::instruction>>& back_track) :
			root(root),
			back_track(std::move(back_track))
		{ }

		explicit loop_unrolled_bst(ir::statement::if_statement* const root) :
			root(root)
		{ }
