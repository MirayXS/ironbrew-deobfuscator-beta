...
namespace deobf::ironbrew_devirtualizer::devirtualizer_markers {
	...
	struct mark_wrapper_visitor final : devirtualizer_markers::marker_decorator {
		...
		bool accept(ir::statement::while_statement* statement) override {
...

			{
				...
				if (result.has_value()) {
					...
					current_block->insert_symbol<true>("__entry_tree__", result_reference.as<ir::statement::if_statement>());
	...
	{
		const auto block = static_cast<ir::statement::block*>(root);
		std::cout << "optimizer\n";
